...
class BasketModelViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response({'product_id': 'Данное поле необходимо'}, status=status.HTTP_400_BAD_REQUEST)
        products = Product.objects.filter(id=product_id)
        if not products.exists():
            return Response({'product_id': 'Объект с данным ID не существует'}, status=status.HTTP_400_BAD_REQUEST)
